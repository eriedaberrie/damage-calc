...
function getEndOfTurn(gen, attacker, defender, move, field) {
    ...
    if (field.hasWeather('Sun', 'Harsh Sunshine')) {
        ...
        if (field.hasWeather('Thunderstorm')) {
            if (!defender.hasType('Electric') &&
                !defender.hasAbility('Volt Absorb', 'Motor Drive', 'Lightning Rod')) {
                var eff = gen.types.get('electric').effectiveness[defender.types[0]] *
                    (defender.types[1] ? gen.types.get('electric').effectiveness[defender.types[1]] : 1);
                damage -= Math.floor(defender.maxHP() * eff / 8);
                texts.push('thunderstorm damage');
            }
        }
        else if (field.hasWeather('Toxic Fallout')) {
            if (!defender.hasType('Nuclear') && !defender.hasType('Steel') &&
                !defender.hasAbility('Lead Skin') &&
                !defender.hasItem('Safety Goggles')) {
                var eff = gen.types.get('nuclear').effectiveness[defender.types[0]] *
                    (defender.types[1] ? gen.types.get('nuclear').effectiveness[defender.types[1]] : 1);
                damage -= Math.floor(defender.maxHP() * eff / 8);
