...
$(".ability").bind("keyup change", function () {
	...
	var TOGGLE_ABILITIES = ['Flash Fire', 'Intimidate', 'Minus', 'Plus', 'Slow Start', 'Unburden', 'Stakeout', 'Petrify'];
...
function autosetWeather(ability, i) {
	...
	case "Stormbringer":
		...
		break;
	case "Chernobyl":
		lastAutoWeather[i] = "Toxic Fallout";
...
$(".set-selector").change(function () {
	...
	if (pokemon) {
		...
		if (regSets || randset) {
			...
			setSelectValueIfValid(abilityObj, pokemon.ab, abilityFallback);
			itemObj.val("");
			for (i = 0; i < 4; i++) {
				moveObj = pokeObj.find(".move" + (i + 1) + " select.move-selector");
				moveObj.attr('data-prev', moveObj.val());
...
function createField() {
  ...
  var isWeatherOff = $("#weatherdamageoff").prop("checked");
	...
	return new calc.Field({
		gameType: gameType, weather: weather, terrain: terrain, isMagicRoom: isMagicRoom, isWonderRoom: isWonderRoom, isGravity: isGravity, isWeatherOff: isWeatherOff,
...
$(document).ready(function () {
	var params = new URLSearchParams(window.location.search);
	var g = GENERATION[params.get('gen')] || 6;
